...
continue_one=input('Now that we have the sequences downloaded, do you want to continue to see the sequence conservation results? Please return in y or n:\n')
...
if continue_two.lower()=='y':
	...
	names=[]
	inputseq=open('inputseqs.fasta').read()
	inputseq=inputseq.split('>')
	length=len(inputseq)
	for a in range(1,length):
		names.append(inputseq[a].split(' ')[0])
	names=set(names)
	for name in names:
	...
	print('The files containing motifs of sequences are in the patmatmotif folder.And I have made a txt file containing sequence ids,motif numbers and motif names called motif_summary.txt which is also in the folder.')
